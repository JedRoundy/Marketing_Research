...
conjoint_prompt <- function(brand_1, type_1, fat_1, package_1, price_1, brand_2, type_2, fat_2, package_2, price_2){
...
for (i in seq(1, 7, by = 2)) {
    task <- d[i:(i+1), ]
    args <- list(d[1,'brand_name'], d[1, 'type_name'], d[1, 'fat_name'], d[1, 'package_name'], d[1, 'price_name'], d[2,'brand_name'], d[2, 'type_name'], d[2, 'fat_name'], d[2, 'package_name'], d[2, 'price_name'])
    cp <- do.call(conjoint_prompt, args = args)
    role <- cp[1]
    prompt <- cp[2]
    
    get_response(n = 5, chat = TRUE, prompt = prompt, role = role, keywords = c('1', '2','3','4'))
